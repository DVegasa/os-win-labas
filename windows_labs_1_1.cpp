/*
Напишите программу, которая запрашивает у пользователя диск (то есть, его букву: «a», «b», «c» 
и т.д. ) и, ес-ли он существует (GetLogicalDrives), выводит тип носителя (GetDriveType), информацию 
о диске (GetVolumeIn-formation), о свободном месте на нем (GetDiskFreeSpace). Соответственно, 
если диска не существует, выводит-ся поясняющее сообщение.
*/
